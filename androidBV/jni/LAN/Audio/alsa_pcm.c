...
struct pcm *pcm_open(int SampleRate, unsigned flags)
{
    const char *dname;
    struct pcm *pcm;
    ...
    unsigned period_sz;
    unsigned period_cnt;
	//char value[PROPERTY_VALUE_MAX] = {'\0'};//x210

    printf("pcm_open(0x%08x)",flags);
...
//  printf("pcm_open() SampleRate %d,period_cnt %d, period_sz %d, channels %d", SampleRate, period_cnt, period_sz, (flags & PCM_MONO) ? 1 : 2);
//    printf("flags & PCM_MONO=%d\n",flags & PCM_MONO);
    param_init(&params);
