...
typedef struct fmpeg4_parm{
	...
	typedef struct
	{
		...
		unsigned int u32VirtualBaseAddr;
		void * pvSemaphore;
		...
		unsigned short * pu16ACDC;				// virtual address.
		...
		unsigned int * pu32Mbs_phy;			// physical address. buffer for Macroblocks
		unsigned int * pu32Mbs;				// virtual address.
				// Don't care when pfnDmaMalloc != NULL
				// 8 Byte boundary
				// byte size = ((width + 15)/16) x 64
		#if 0
