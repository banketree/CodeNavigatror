...
static int rollingfile_open(log4c_appender_t* this) {
	...
	if (rfup == NULL) {
		...
		sd_debug(
				"making new rollingfile conf object, with default logdir/logprefix");
		rfup = rollingfile_make_udata();

		rollingfile_udata_set_logdir(rfup, ROLLINGFILE_DEFAULT_LOG_DIR);
		rollingfile_udata_set_files_prefix(rfup,
				ROLLINGFILE_DEFAULT_LOG_PREFIX);

	} else {
		sd_debug("appender has udata already: logdir='%s' logprefix='%s'",
				rollingfile_udata_get_logdir(rfup),    //appender logdir
				rollingfile_udata_get_files_prefix(rfup));  //appender prefix
	}
	...
	//basename=logdir+prefix
	rfup->rfu_base_filename = rollingfile_make_base_name(
			rfup->rfu_conf.rfc_logdir, rfup->rfu_conf.rfc_files_prefix);
	...
	if (rfup->rfu_conf.rfc_policy) {
		...
		if (!log4c_rollingpolicy_is_initialized(rfup->rfu_conf.rfc_policy)) {
			...
			log4c_rollingpolicy_init(rfup->rfu_conf.rfc_policy, rfup);
		}
		...
		rollingfile_open_zero_file(rfup->rfu_base_filename,
				&rfup->rfu_current_file_size, &rfup->rfu_current_fp);
	}
...
static int rollingfile_append(log4c_appender_t* this,
	...
	rollingfile_udata_t* rfup = log4c_appender_get_udata(this);
	//log4c_logging_event_t* my_event = (log4c_logging_event_t*) a_event;
	int rc = 0;
	...
	if (rfup->rfu_conf.rfc_policy != NULL) {
		...
		if (log4c_rollingpolicy_is_triggering_event(rfup->rfu_conf.rfc_policy,
				a_event, rfup->rfu_current_file_size)) {
#ifdef __SD_DEBUG__
...
LOG4C_API int rollingfile_udata_set_logdir(rollingfile_udata_t* rfup,
		char *logdir) {
...
LOG4C_API int rollingfile_udata_set_files_prefix(rollingfile_udata_t* rfup,
		char* fprefix) {
...
//	if(iFileBuffer > 3000 || iFileBuffer == 0)
//	{
//		log4c_set_file_buffer(0);
//		setbuf(*fpp, NULL);
//		printf("debug:log file buffer size:%d\n", iFileBuffer);
//	}
//	else
//	{
//		//iFileBuffer == FILE_BUFFER_DEAULT;
//		setbuf(*fpp, iFileBuffer);
//		printf("debug:log file buffer size:%d\n", iFileBuffer);
//	}
...
static char *rollingfile_make_base_name(const char *logdir, const char* prefix) {
	...
	filename_len = strlen(logdir) + 1 + strlen(prefix) + 1 + 10; /* a margin */

	s = (char *) malloc(filename_len);
	sprintf(s, "%s%s%s", logdir, FILE_SEP, prefix);

	return (s);
}

/****************************************************************************/
const log4c_appender_type_t log4c_appender_type_rollingfile = { "rollingfile",
		rollingfile_open, rollingfile_append, rollingfile_close };
