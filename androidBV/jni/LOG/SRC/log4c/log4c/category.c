  ...
  va_list a_args)
{
  ...
  */
  ...
  if (!evt.evt_buffer.buf_maxsize) {
    ...
    if ( (n = (size_t)vsnprintf(message, evt.evt_buffer.buf_size, a_format, a_args)) >= evt.evt_buffer.buf_size)
    	sd_error("truncating message of %d bytes (bufsize = %d)", n, evt.evt_buffer.buf_size);
  }
  ...
  for (cat = this; cat; cat = cat->cat_parent) {
    if (cat->cat_appender)
    {
    	...
    	//log4c_appender_append(cat->cat_appender, &evt); //原先

    	//--------------------------------------------------------------------------------
    	//以下是替换后的代码 modify by chensq
    	//原先只根据最小的类型的等级定义，父类型等级定义无效，目前修改为父类型同时有效
    	if( log4c_category_is_priority_enabled(cat, a_priority))
    	{
//    		printf("debug:will append(define:%d true:%d)\n", cat->cat_priority, a_priority);
    		log4c_appender_append(cat->cat_appender, &evt);
    	}
    	else
    	{
//    		printf("debug:difine priority level too low, so will not append(define:%d true:%d)\n", cat->cat_priority, a_priority);
    	}
    	//--------------------------------------------------------------------------------
