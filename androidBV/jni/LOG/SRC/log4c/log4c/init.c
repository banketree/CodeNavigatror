...
#include "LOGIntfLayer.h"//use LOG4C_RCPATH
...
{    
    ...
    {
    	 sd_debug("layout_types[%d].name=%s",  i, layout_types[i]->name); //add by chensq
    ...
    {
    	 sd_debug("appender_types[%d].name=%s",  i, appender_types[i]->name); //add by chensq
    ...
    {
    	 sd_debug("rollingpolicy_types[%d].name=%s",  i, rollingpolicy_types[i]->name); //add by chensq
	...
	for (i = 0; i < nrcfiles; i++) {
	    ...
	    if (SD_ACCESS_READ(rcfiles[i].name)) {
	    	sd_debug("conf file is not existed ('%s')", rcfiles[i].name); //add by chensq
	    	continue;
	    }
	    else
	    {
	    	sd_debug("conf file is existed ('%s')", rcfiles[i].name);	//add by chensq
	    }
	    ...
	    if (log4c_load(rcfiles[i].name) == -1)
	    {
			...
			printf("LG loading %s failed[LOG4C_RCPATH:%s]\n", rcfiles[i].name, LOG4C_RCPATH);
			ret = -1;
	    }
	    else
	    {
	    	sd_debug("loading %s succeeded", rcfiles[i].name);
	    	printf("LG loading %s succeeded[LOG4C_RCPATH:%s]\n", rcfiles[i].name, LOG4C_RCPATH);
	    }
