...
static log4c_rc_t __log4c_rc = { 0 };

log4c_rc_t* const log4c_rc = &__log4c_rc;


//add by chensq 20121115
int log4c_get_file_buffer()
{
	return log4c_rc->config.file_buffer;
}

void log4c_set_file_buffer(int value)
{
	log4c_rc->config.file_buffer = value;
}

//add by chensq 20120730
int log4c_get_upload_mode()
{
	return log4c_rc->upload.mode;
}

int log4c_get_upload_hour()
{
	return log4c_rc->upload.hour;
}

int log4c_get_upload_minute()
{
	return log4c_rc->upload.minute;
}

int log4c_get_upload_maxspace()
{
	return log4c_rc->upload.maxspace;
}

int log4c_get_upload_cycle_time()
{
	return log4c_rc->upload.cycle_time;
}
...
static int config_load(log4c_rc_t* this, sd_domnode_t* anode)
{
	...
	if (!strcmp(node->name, "debug")) {
    	...
    	if (!strcmp(node->name, "file_buffer")) {
    	    this->config.file_buffer = parse_byte_size(node->value);

    	    if (this->config.file_buffer)
    		sd_debug("using file_buffer size of %d Bytes",
    			 this->config.file_buffer);
    	    else
    		sd_debug("using default  file_buffer");
    	}
...
//add by chensq 20120730
/******************************************************************************/
static int upload_load(log4c_rc_t* this, sd_domnode_t* anode)
{
    sd_list_iter_t* i = NULL;
	 ...
	 i = sd_list_iter_next(i))
    {
		...
		if (!strcmp(node->name, "hour"))
		{
			this->upload.hour = atoi(node->value);
			sd_debug("==========LG: upload_load hour %d", this->upload.hour);
		}
		if (!strcmp(node->name, "minute"))
		{
			this->upload.minute = atoi(node->value);
			sd_debug("==========LG: upload_load minute %d", this->upload.minute);
		}
		if (!strcmp(node->name, "maxspace"))
		{
			this->upload.maxspace = atoi(node->value);
			sd_debug("==========LG: upload_load maxspace %d", this->upload.maxspace);
		}
		//add by lcx 2013.2.26
		if(!strcmp(node->name, "cycle_time"))
		{
			this->upload.cycle_time = atoi(node->value);
			sd_debug("==========LG: upload_load cycle_time %d", this->upload.cycle_time);
			printf("==========LG: upload_load cycle_time %d", this->upload.cycle_time);
		}
		if(!strcmp(node->name, "mode"))
		{
			this->upload.mode = atoi(node->value);
			sd_debug("==========LG: upload_load mode %d", this->upload.mode);
		}
...
{    
  ...
  if (!strcmp(node->name, "rollingpolicy"))rollingpolicy_load(this, node);
	...
	//add by chensq
	if (!strcmp(node->name, "upload"))   upload_load(this, node);
