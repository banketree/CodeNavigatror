...
/* Command handlers */
static int lpc_cmd_log(LinphoneCore *, char *);
static int lpc_cmd_transdir(LinphoneCore *, char *);
...
void linphonec_codec_enable(int type, LinphoneCore *lc, int index);
void linphonec_codec_disable(int type, LinphoneCore *lc, int index);



/* Command table management */
static LPC_COMMAND *lpc_find_command(const char *name);
static int lpc_cmd_dtmf(LinphoneCore *lc, char *args);
...
static LPC_COMMAND commands[] = {
	//add by chensq 20121102
	{ "log", lpc_cmd_log, "use some param can show log.",
		"'log enable'\t: shows all linphone logs.\n"
		"'log disable'\t: don't show all linphone logs.\n"
	},
	...
	//SalStreamInactive
	//SalStreamDir;
	//add by chensq 20121220
	{ "transdir", lpc_cmd_transdir, "use some param can set transdir.",
		"'transdir audio sendrecv/sendonly/recvonly/inactive'\t: set audio transdir.\n"
		"'transdir video sendrecv/sendonly/recvonly/inactive'\t: set audio transdir.\n"
	},
	//add by chensq 20121102
	{ "dtmf", lpc_cmd_dtmf, "send dtmf info.",
		"'dtmf 1'\t: send dtmf 1.\n"
	},
...
//add by chensq 20121102
static unsigned int oldLevelMask = 9999;
static int
lpc_cmd_log(LinphoneCore *lc, char *args)
{
	if(oldLevelMask == 9999)
	{
		oldLevelMask = ortp_get_log_level_mask();
	}
	if ( ! args )
	{
		return 1;
	...
	if (strstr(args,"enable")){
		...
		ortp_set_log_level_mask(oldLevelMask);
	}
	else return 0;
...
//add by chensq 20121220
//transdir audio sendrecv/sendonly/recvonly/inactive
SalStreamDir g_enAudioDir = SalStreamSendRecv;
SalStreamDir g_enVideoDir = SalStreamSendRecv;
static int
lpc_cmd_transdir(LinphoneCore *lc, char *args)
{
	char *arg1 = args;
	char *arg2 = NULL;
	char *ptr = args;
	...
	if ( *ptr )
	{
		*ptr='\0';
		...
		linphonec_out("%s ==> %s\n", arg1, arg2);
		if(strcmp(arg1, "audio") == 0)
		{
			if(strcmp(arg2, "sendrecv") == 0)
			{
				g_enAudioDir = SalStreamSendRecv;
			}
			else if(strcmp(arg2, "sendonly") == 0)
			{
				g_enAudioDir = SalStreamSendOnly;
			}
			else if(strcmp(arg2, "recvonly") == 0)
			{
				g_enAudioDir = SalStreamRecvOnly;
			}
			else if(strcmp(arg2, "inactive") == 0)
			{
				g_enAudioDir = SalStreamInactive;
			}
		}
		else if(strcmp(arg1, "video") == 0)
		{
			if(strcmp(arg2, "sendrecv") == 0)
			{
				g_enVideoDir = SalStreamSendRecv;
			}
			else if(strcmp(arg2, "sendonly") == 0)
			{
				g_enVideoDir = SalStreamSendOnly;
			}
			else if(strcmp(arg2, "recvonly") == 0)
			{
				g_enVideoDir = SalStreamRecvOnly;
			}
			else if(strcmp(arg2, "inactive") == 0)
			{
				g_enVideoDir = SalStreamInactive;
			}
		}
	...
	else
	{
		...
		linphonec_out("missing one parameter\n");
		return 0;
	}
...
static int
lpc_cmd_dtmf(LinphoneCore *lc, char *args)
{
	if ( ! args )
	{
		return 0;
	...
	if(LinphoneCallStreamsRunning == call_state)
	{
		LOG_RUNLOG_DEBUG("LP send dtmf %c", args[0]);
		linphonec_out("linphonec_send_dtmf:args:%c\n", args[0]);
		linphonec_send_dtmf(args[0]);
	}
	else
	{
...
lpc_cmd_call(LinphoneCore *lc, char *args)
{
	...
	}
	{
		...
		//modify by chensq for hint music
		//char *opt1,*opt2;
		char *opt1,*opt2,*opt3;
...
#ifdef HINT_MUSIC
		//add by chensq for hint music
		opt3=strstr(args,"--hint_music_start");
#endif
		...
		if (opt3)
		{
			char hint_music[MAX_INPUT];
			char *file_start = opt3 + strlen("--hint_music_start");
			char *file_end = strstr(file_start, "--hint_music_end");
			assert(file_end - file_start < MAX_INPUT);
			*file_end = '\0';

			strcpy(hint_music, file_start);
			linphone_core_set_hint_music(cp, hint_music);
			*opt3 = '\0';
		}
#endif
		if ( NULL == (call=linphone_core_invite_with_params(lc, args,cp)) )
		{
	    	...
	    	LOG_RUNLOG_ERROR("LP Error from linphone_core_invite");
	    	LOG_EVENTLOG_INFO(LOG_EVENTLOG_FORMAT, LOG_EVENTLOG_NO_ROOM, LOG_EVT_APP_REBOOT, 0, "");
	    	LOG_EVENTLOG_INFO(LOG_EVENTLOG_FORMAT, LOG_EVENTLOG_NO_ROOM, LOG_EVT_SYSTEM_ERROR, 0, "109");
...
//add by chensq
static void linphonec_proxy_remove_all(LinphoneCore *lc)
{
    ...
    {
        cfg = (LinphoneProxyConfig*) ms_list_nth_data(proxies, n);
        if (cfg)
        {
            linphone_core_remove_proxy_config(lc, cfg);
        }
    }
    linphonec_out("Proxy removed all.\n");
}
...
//    while (1)
//    {
//        char *input=linphonec_readline("Enter proxy sip authorization: ");
//        char *clean;
//
//        if ( ! input ) {
//            linphonec_out("Aborted.\n");
...
//
//        cfg=linphone_proxy_config_new();
//        if (linphone_proxy_config_set_identity(cfg,clean)/*linphone_proxy_config_set_server_addr(cfg,clean)<0*/)
//        {
//            linphonec_out("Invalid sip address (sip:sip.domain.tld).\n");
//            free(input);
//            linphone_proxy_config_destroy(cfg);
...
//    while (1)
//    {
//        char *input=linphonec_readline("Specify sip proxy address: ");
//        char *clean;
//
//        if ( ! input ) {
//            linphonec_out("Aborted.\n");
...
//            break;
//        }
//        linphone_proxy_config_set_server_addr(cfg, clean);
//        linphone_proxy_config_set_route(cfg, clean);
//        if ( ! linphone_proxy_config_get_route(cfg) )
//        {
//            linphonec_out("Invalid route.\n");
...
//    while (1)
//    {
//        char *input=linphonec_readline("Your password for this proxy: ");
//        char *clean;
//
//        if ( ! input ) {
//            linphonec_out("Aborted.\n");
...
//            continue;
//        }
//        if ((from=linphone_address_new(linphone_proxy_config_get_identity(cfg)))!=NULL){
//            char realm[128];
//            snprintf(realm,sizeof(realm)-1,"\"%s\"",linphone_address_get_domain(from));
//            info=linphone_auth_info_new(linphone_address_get_username(from),username,clean,NULL,NULL);
...
static int lpc_cmd_register(LinphoneCore *lc, char *args){
    ...
    passwd[0]=proxy[0]=identity[0]='\0';
    sscanf(args,"%s %s %s %s",authorization, proxy, identity, passwd);
    if (authorization[0]=='\0' || proxy[0]=='\0'){
        linphonec_out("Missing parameters, see help register\n");
        return 1;
    }
    printf("authorization:%s proxy:%s identity:%s passwd:%s\n",authorization, proxy, identity, passwd);
    if (identity[0]!='\0' && passwd[0]!='\0'){
        LinphoneAddress *from;
        LinphoneAuthInfo *info;
        if ((from=linphone_address_new(authorization))!=NULL){
            char realm[128];
            snprintf(realm,sizeof(realm)-1,"\"%s\"",linphone_address_get_domain(from));
            info=linphone_auth_info_new(linphone_address_get_username(from),identity,passwd,NULL,NULL);
            linphone_core_add_auth_info(lc,info);
            linphone_address_destroy(from);
            linphone_auth_info_destroy(info);
        }
    ...
    linphone_proxy_config_set_identity(cfg,authorization);
    linphone_proxy_config_set_server_addr(cfg,proxy);
    linphone_proxy_config_set_route(cfg,proxy);
    linphone_proxy_config_enable_register(cfg,TRUE);
    if (elem) linphone_proxy_config_done(cfg);
    else linphone_core_add_proxy_config(lc,cfg);
    linphone_core_set_default_proxy(lc,cfg);
...
void linphonec_codec_enable(int type, LinphoneCore *lc, int sel_index){
	PayloadType *pt;
	int index=0;
	const MSList *node=NULL;
...
void linphonec_codec_disable(int type, LinphoneCore *lc, int sel_index){
	PayloadType *pt;
	int index=0;
	const MSList *node=NULL;
...
////----------------------------------------
////add by chensq
//int linphonec_proxy_add2(LinphoneCore *lc, const char* username,
//        const char* password, const char* proxy, const char* route, int expires,
//        int enable_register)
//{
...
//    LinphoneAddress* from;
//    char realm[128] = { 0x0 };
//    char auth_info_user[128] = { 0x0 };
//
//    linphonec_proxy_remove_all(lc);
//
//    while (cfg)
//    {
//        clean = lpc_strip_blanks(username);
//        if (linphone_proxy_config_set_identity(cfg, clean) < 0)
//        {
//            break;
//        }
//
//        clean = lpc_strip_blanks(proxy);
//        if (linphone_proxy_config_set_server_addr(cfg, clean) < 0)
//        {
//            break;
//        }
...
//        linphone_proxy_config_enableregister(cfg, enable_register);
//
//        clean = lpc_strip_blanks(route);
//        if (linphone_proxy_config_set_route(cfg, clean) < 0)
//        {
//            break;
//        }
//
//        if (linphone_core_add_proxy_config(lc, cfg) < 0)
//        {
//            break;
//        }
//
//        //linphone_core_set_sip_port(lc, 5060);      // add by wzk
//        linphone_core_set_default_proxy(lc, cfg);
//
//
//        if ((from = linphone_address_new(username)) == NULL)
//        {
//            break;
//        }
//        snprintf(realm, sizeof(realm) - 1, "\"%s\"", linphone_address_get_domain(from));
//
////        //if (strcmp(realm, "\"fj.ctcims.cn\"") == 0)
////        if ( strcmp(realm, "\"fj.ctcims.cn\"") == 0 || strcmp(realm, "\"zte.com\"") == 0
////        		|| strcmp(realm, "\"gd.ctcims.cn\"") == 0 || strcmp(realm, "\"gd.ctcims.cn\"") == 0)
////        {
////            sprintf(auth_info_user, "%s", username + strlen("sip:"));
////        }
////        else
////        {
//            sprintf(auth_info_user, "%s", linphone_address_get_username(from));
////        }
//        printf("debug:auth info user : %s\n", auth_info_user);
//        info = linphone_auth_info_new(auth_info_user, NULL, password, NULL, NULL);
//        linphone_core_add_auth_info(lc, info);
//        linphone_address_destroy(from);
//        linphone_auth_info_destroy(info);
//        return 0;
...
int linphonec_proxy_add2(LinphoneCore *lc, const char* username,
        ...
        int enable_register)
{
    bool_t bEnableRegister=TRUE;
    LinphoneProxyConfig *cfg;

    LinphoneAddress *from;
    LinphoneAuthInfo *info;
    char acIentity[512]={0X0};
    char acUserName[512]={0X0};
    char *clean;
    ...
    */
	...
	clean=lpc_strip_blanks(username);
	cfg=linphone_proxy_config_new();
	sprintf(acIentity, "%s%s", "sip:", clean);
	//printf("linphonec_proxy_add2:identity=%s\n", clean);
	if (linphone_proxy_config_set_identity(cfg,acIentity)/*linphone_proxy_config_set_server_addr(cfg,clean)<0*/)
	{
		linphonec_out("Invalid sip address (sip:sip.domain.tld).\n");
		linphone_proxy_config_destroy(cfg);
		return -1;
	}
	//printf("linphonec_proxy_add2:linphone_proxy_config_get_identity=%s\n", linphone_proxy_config_get_identity(cfg));
    ...
    */
	...
	//printf("linphonec_proxy_add2:server_addr/route=%s\n", clean);

    clean=lpc_strip_blanks(username);
    sprintf(acUserName, "%s", clean);

	char *pcTmp = strstr(acUserName, "ctcims");
    if(pcTmp == NULL)
    {
    	//printf("===================account is not ctcims\n");
        pcTmp = strchr(acUserName, '@');
//        if( (*(pcTmp+1)) <= '9' && (*(pcTmp+1)) >= '0' )
//        {
        	*pcTmp = '\0';
//        }
    }
    ...
    //char *pcTmp = strchr(acUserName, '@');
    //if( (*(pcTmp+1)) <= '9' && (*(pcTmp+1)) >= '0' )
    //{
    //	*pcTmp = '\0';
    //}
    //printf("linphonec_proxy_add2:acUserName=%s\n", acUserName);
    ...
    */
	/* Strip blanks */
	clean=lpc_strip_blanks(password);
	if ((from=linphone_address_new(linphone_proxy_config_get_identity(cfg)))!=NULL){
		char realm[128];
		snprintf(realm,sizeof(realm)-1,"\"%s\"",linphone_address_get_domain(from));
		info=linphone_auth_info_new(linphone_address_get_username(from),acUserName,clean,NULL,NULL);
		linphone_core_add_auth_info(lc,info);
		linphone_address_destroy(from);
		linphone_auth_info_destroy(info);
	}
