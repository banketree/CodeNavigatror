...
//modify by chensq support two version 20120521
...
void linphone_core_update_streams(LinphoneCore *lc, LinphoneCall *call, SalMediaDescription *new_md){
...
#ifndef HINT_MUSIC
		//annotate by chensq for hint music
		//idbt restart it every time for hint music
...
#endif
		//LOG_RUNLOG_DEBUG("LP linphone_core_update_streams will stop media streams");
	...
	if (new_md) {
		...
		//---------------------
#ifdef HINT_MUSIC
		//add by chensq for hint music 主叫，且被叫已摘机。此时发送提示音
		if (LinphoneCallConnected == call->state &&
			LinphoneCallOutgoing == call->dir &&
			0 != strlen(call->params.hint_music))
		{
			all_muted = TRUE;
			send_ringbacktone = TRUE;
		}
...
static void call_failure(SalOp *op, SalError error, SalReason sr, const char *details, int code){
    ...
    if(lc->vtable.notify_call_failure)
    {
        lc->vtable.notify_call_failure(lc, call, error, sr, code);
    }
    //--------------------------------------------------------------------------------
...
//	LinphoneAuthInfo *ai=(LinphoneAuthInfo*)linphone_core_find_auth_info(lc,realm,username);
    ...
    //add by chensq 20120524 同时支持IMS SIP服务器与普通SIP服务器
    LinphoneCore *lc=NULL;
    LinphoneAuthInfo *ai=NULL;
//    if( strncmp("\""LP_IMS_SERVER_REALM"\"", realm, strlen(LP_IMS_SERVER_REALM+2)) == 0 ||
//    		strncmp("\""LP_IMS_SERVER_REALM_GZ"\"", realm, strlen(LP_IMS_SERVER_REALM_GZ+2)) == 0 ||
//    		strncmp("\""LP_IMS_SERVER_REALM_GZ2"\"", realm, strlen(LP_IMS_SERVER_REALM_GZ2+2)) == 0)
//    {
//        //IMS服务器
//        char new_username[128]={0x0};
//        char *realm_dup = osip_strdup_without_quote(realm);
//        sprintf(new_username, "%s@%s", username, realm_dup);
//        osip_free(realm_dup);
//        lc=(LinphoneCore *)sal_get_user_pointer(sal_op_get_sal(h));
//        ai=(LinphoneAuthInfo*)linphone_core_find_auth_info(lc,realm,new_username);
//        //printf("debug:use ims type[auth_requested:%d]\n", h->expires);
//    }
//    else
//    {
        //普通服务器
       ...
       ai=(LinphoneAuthInfo*)linphone_core_find_auth_info(lc,realm,username);
        //printf("debug:use normal type[auth_requested:%d]\n", h->expires);
//    }
	...
	if (ai && (ai->works || ai->usecount<3)){
		...
		//ai->usecount++;   //取消ai->usecount<3限制 20120529
	}else{
...
//	//add by chensq 20120521
//    if (lc->vtable.notify_reg_status)
//    {
//        lc->vtable.notify_reg_status(lc, TRUE, 0);
//    }
...
//    if (lc->vtable.notify_reg_status)
//    {
//        lc->vtable.notify_reg_status(lc, FALSE, status_code);
//    }
