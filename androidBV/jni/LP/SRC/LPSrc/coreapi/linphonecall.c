...
//该文件暂未对SIP号码无法呼叫时候，出现的8126视频驱动异常问题进行处理 by chensq 20120525
...
//20120529 dtmf inband
static void ms_filter_inband_dtmf_receive(void *userdata, struct _MSFilter *f, unsigned int id, void *arg);
...
extern SalStreamDir g_enAudioDir;
extern SalStreamDir g_enVideoDir;
...
static void post_configure_audio_streams(LinphoneCall*call){
	...
	//20120529 dtmf inband
	if(st->dtmfinband!=NULL){	
		ms_filter_set_notify_callback(st->dtmfinband, ms_filter_inband_dtmf_receive, call);
	}
}
...
#ifdef HINT_MUSIC
	LOG_RUNLOG_DEBUG("LP HINT_MUSIC setup_ring_player");
	//---------------------
	//add by chensq for hint music
	//主叫发送提示音功能
	if (LinphoneCallConnected == call->state &&
		LinphoneCallOutgoing == call->dir)
	{
		printf("debug:setup_ring_player hint_music(%s)\n", call->params.hint_music);
		assert(0 != strlen(call->params.hint_music));
		/* -2表示只播放一次 */
		pause_time = -2;
		LOG_RUNLOG_DEBUG("LP HINT_MUSIC audio_stream_play(%s)", call->params.hint_music);
		audio_stream_play(call->audiostream,call->params.hint_music);
		ms_filter_call_method(call->audiostream->soundread,MS_FILE_PLAYER_LOOP,&pause_time);
	}
	//---------------------
	else
...
#endif
		//src code
		//printf("debug:setup_ring_player ringback_tone(%s)\n", lc->sound_conf.ringback_tone);
		audio_stream_play(call->audiostream,lc->sound_conf.ringback_tone);
		ms_filter_call_method(call->audiostream->soundread,MS_FILE_PLAYER_LOOP,&pause_time);
#ifdef HINT_MUSIC
	}
...
static void ms_filter_inband_dtmf_receive(void *userdata, struct _MSFilter *f, unsigned int id, void *arg)
{
	...
	if (lc->vtable.dtmf_received != NULL)
	{
		lc->vtable.dtmf_received(lc, call, *(char*)arg);
	}
}

#ifdef HINT_MUSIC
//add by chensq for hint music
static void ms_filter_hint_music_done(void *userdata, struct _MSFilter *f, unsigned int id, void *arg)
{
	LOG_RUNLOG_DEBUG("LP HINT_MUSIC ms_filter_hint_music_done");
    LinphoneCall *call = (LinphoneCall *)userdata;

    //如果是主叫为被叫播放提示音，播放完毕后改用mic作为输入
    if (LinphoneCallStreamsRunning == call->state &&
    	LinphoneCallOutgoing == call->dir)
    {
    	LOG_RUNLOG_DEBUG("LP HINT_MUSIC ms_filter_hint_music_done will stop media streams");
    	linphone_call_stop_media_streams(call);
    	linphone_call_init_media_streams(call);
    	linphone_call_start_media_streams(call, FALSE, FALSE);

    	/* 提示上层提示音播放完毕 */
    	LinphoneCore *cl = call->core;
    	LOG_RUNLOG_DEBUG("LP HINT_MUSIC ms_filter_hint_music_done will exec callback outgoing_hint_music_done");
    	cl->vtable.outgoing_hint_music_done(cl, call);
    }
}


//add by chensq for hint music
static void install_ring_done_notify(LinphoneCall *call, bool_t send_ringbacktone)
{
    if (send_ringbacktone)
    {
        MSFilter *f = call->audiostream->soundread;
        ms_filter_set_notify_callback(f, ms_filter_hint_music_done, call);
    }
}
#endif
...
void linphone_call_start_media_streams(LinphoneCall *call, bool_t all_inputs_muted, bool_t send_ringbacktone){
	...
	cname=linphone_address_as_string_uri_only(me);
	{
		...
		if (stream && stream->dir!=SalStreamInactive){
			...
			//printf("====================playcard name:%s\n", playcard->name);
			MSSndCard *captcard=lc->sound_conf.capt_sndcard;
			//printf("====================captcard name:%s\n", captcard->name);
			const char *playfile=lc->play_file;
			//printf("====================playfile name:%s\n", playfile);
			const char *recfile=lc->rec_file;
			//printf("====================recfile name:%s\n", recfile);
...
#ifdef HINT_MUSIC
				//add by chensq for hint music
				install_ring_done_notify(call, send_ringbacktone);
				LOG_RUNLOG_DEBUG("LP HINT_MUSIC: install ring done notify");
#endif
...
#ifdef VIDEO_ENABLED
	if(call->state!=LinphoneCallOutgoingEarlyMedia)// add by chensq
	{
		//printf("-------------------- LC video:call->state!=LinphoneCallOutgoingEarlyMedia[call->videostream==NULL:%d]\n", call->videostream==NULL);
		{
