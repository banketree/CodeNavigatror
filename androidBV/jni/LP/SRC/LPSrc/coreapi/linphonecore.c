...
void linphone_core_disable_logs(){
	//add by chensq and ORTP_MESSAGE ORTP_WARNING for debug mode src:ORTP_ERROR|ORTP_FATAL
...
static bool_t get_codec(LpConfig *config, const char* type, int index, PayloadType **ret){
	...
	*ret=NULL;
	...
	printf("get_codec:type=%s,index=%d\n",type,index);
	...
	if (pt && enabled ) 
	{
		...
		printf("Found codec %s/%d\n",pt->mime_type,pt->clock_rate);
	}
	if (pt==NULL) 
	{	
		ms_warning("Ignoring codec config %s/%i with fmtp=%s because unsupported",
	    		mime,rate,fmtp ? fmtp : "");
		printf("Ignoring codec config %s/%d with fmtp=%s because unsupported\n",
	    		mime,rate,fmtp ? fmtp : "");
	}
	*ret=pt;
	return TRUE;
		...
		if (pt){
			...
			if (pt && ms_filter_codec_supported(pt->mime_type)){
				if (ms_list_find(l,pt)==NULL){
					...
					LOG_RUNLOG_DEBUG("Adding new codec %s/%i with fmtp %s",
...
static void codecs_config_read(LinphoneCore *lc)
{
	...
	for (i=0;get_codec(lc->config,"video_codec",i,&pt);i++){
		if (pt){
			if (!ms_filter_codec_supported(pt->mime_type)){
				ms_warning("Codec %s is not supported by mediastreamer2, removed.",pt->mime_type);
				printf("Codec %s is not supported by mediastreamer2, removed.\n",pt->mime_type);
...
//----------------------------------------
//modify by chensq 20120525   PC需要恢复
#ifdef VIDEO_ENABLED
	...
	linphone_core_assign_payload_type(&payload_type_mp4v,99,"CIF=1");
#endif

#if defined(_CODEC_H264_)
	linphone_core_assign_payload_type(&payload_type_h264,102,"profile-level-id=42801E;packetization-mode=1;video-mode=1;");
#endif
//----------------------------------------
 ...
 * @ingroup network_parameters
**/
int linphone_core_set_sip_transports(LinphoneCore *lc,  LCSipTransports * tr){
...
//#ifdef _YK_XT8126_BV_
//	//add by wzk
//	while(-1==linphone_core_set_sip_transports (lc,&tr)){
//		printf("===================== linphone_core_set_sip_port try set transports[%d] failed\n", tr.udp_port);
//		tr.udp_port++;
//		if(tr.udp_port-port>2000){
//			break;
//		}
//	}
//#else
...
//	LOG_RUNLOG_DEBUG("bandwidth usage: audio=[d=%.1f,u=%.1f] video=[d=%.1f,u=%.1f] kbit/sec",
//			(as!=NULL) ? (rtp_session_compute_recv_bandwidth(as)*1e-3) : 0,
//			(as!=NULL) ? (rtp_session_compute_send_bandwidth(as)*1e-3) : 0,
//			(vs!=NULL) ? (rtp_session_compute_recv_bandwidth(vs)*1e-3) : 0,
//			(vs!=NULL) ? (rtp_session_compute_send_bandwidth(vs)*1e-3) : 0);
...
static bool_t new_status = FALSE;
bool_t LPGetNetStatus()
{
	//printf("==================debug:LPGetNetStatus:%d\n", new_status);
	return new_status;
}
...
static void monitor_network_state(LinphoneCore *lc, time_t curtime){
	...
	if (last_check==0 || (curtime-last_check)>=5){
		if( LPUtilsCheckNet() == 0 )
		{
			new_status=TRUE;
			if(new_status!=last_status)
			{
				LOG_RUNLOG_DEBUG("LP monitor_network_state check net change to online");
			}
		}
		else
		{
			...
			if(new_status!=last_status)
			{
				LOG_RUNLOG_DEBUG("LP monitor_network_state check net change to offline");
			}
		}
		last_check=curtime;
		if (new_status!=last_status) {
			...
			if(new_status == FALSE)
			{
				LPSetNetworkStatus(NETWORK_STATUS_OFFLINE);
				LPSetSipRegStatus(REG_STATUS_OFFLINE);
			}
			else if(new_status == TRUE)
			{
				LPSetNetworkStatus(NETWORK_STATUS_ONLINE);
			}


			LOG_RUNLOG_DEBUG("LP monitor_network_state set_network_reachable:%d(true=1)", new_status);
...
//		{
//			new_status=TRUE;
//			//printf("==================debug:monitor_network_state true:%d\n", new_status);
//		}
//		else
//		{
//			new_status=FALSE;
//			//add by chensq
//			//printf("==================debug:monitor_network_state false:%d\n", new_status);
//			if(new_status!=last_status)
//			{
//				LOG_RUNLOG_DEBUG("LP g_enCurrReg=>REG_STATUS_OFFLINE(monitor_network_state check[%d])", LPGetSipRegStatus());
//				LPSetSipRegStatus(REG_STATUS_OFFLINE);
////#ifndef LP_ONLY
...
static void monitor_network_state_bak(LinphoneCore *lc, time_t curtime){
	static time_t last_check=0;
	static bool_t last_status=FALSE;
	...
	/* only do the network up checking every five seconds */
	if (last_check==0 || (curtime-last_check)>=5){
		LOG_RUNLOG_DEBUG("last_check==0 || (curtime-last_check)>=5");
		net_status = linphone_core_get_local_ip_for(lc->sip_conf.ipv6_enabled ? AF_INET6 : AF_INET,NULL,result);
		if (strcmp(result,"::1")!=0 && strcmp(result,"127.0.0.1")!=0)
		{
//			net_status = system("ping 218.85.157.99 -c 1");
			LOG_RUNLOG_DEBUG("LP monitor_network_status,result is %s, net_status is %d", result, net_status);
			if(net_status == 0)
			{
				new_status=TRUE;
				if(new_status!=last_status)
				{
					LOG_RUNLOG_DEBUG("LP g_enCurrNetworkStatus=>NETWORK_STATUS_ONLINE(monitor_network_state check[%d])", NETWORK_STATUS_ONLINE);

					//网络连接的情况下，并不能说明SIP也是注册上了，所以这里不能同时设置SIP注册上
					LPSetNetworkStatus(NETWORK_STATUS_ONLINE);
				}
			}
			else
			{
				...
				if(new_status!=last_status)
				{
					LOG_RUNLOG_DEBUG("LP g_enCurrNetworkStatus=>NETWORK_STATUS_OFFLINE(monitor_network_state check[%d])", NETWORK_STATUS_OFFLINE);
					LPSetNetworkStatus(NETWORK_STATUS_OFFLINE);
					//如果网络没有注册上，则SIP也必将是掉线
					LOG_RUNLOG_DEBUG("LP g_enCurrReg=>REG_STATUS_OFFLINE(monitor_network_state check[%d])", LPGetSipRegStatus());
					LPSetSipRegStatus(REG_STATUS_OFFLINE);
				}
			}
		...
		else
		{
			new_status=FALSE;
			//add by chensq
			LOG_RUNLOG_DEBUG("debug:monitor_network_state false,result is %s", result);
			if(new_status!=last_status)
			{
				LOG_RUNLOG_DEBUG("LP g_enCurrNetworkStatus=>NETWORK_STATUS_OFFLINE(monitor_network_state check[%d])", NETWORK_STATUS_OFFLINE);
				LPSetNetworkStatus(NETWORK_STATUS_OFFLINE);
...
void linphone_core_isable_network_monitor(LinphoneCore* lc,bool_t isable)
{
	lc->auto_net_state_mon = isable;
}
...
LinphoneCallParams *linphone_core_create_default_call_parameters(LinphoneCore *lc){
	...
	memset(p->hint_music, 0, MAX_INPUT);
#endif
	return p;
}
...
#ifdef HINT_MUSIC
//add by chensq for hint music
void linphone_core_set_hint_music(LinphoneCallParams *param, char *hint_music)
{
	strcpy(param->hint_music, hint_music);
}
//add by chensq for hint music
bool_t linphone_core_is_hint_music_none(LinphoneCall *call)
{
	LOG_RUNLOG_DEBUG("LP linphone_core_is_hint_music_none:%s(%d)",  call->params.hint_music, strlen( call->params.hint_music));
	return strlen(call->params.hint_music) ? FALSE : TRUE;
}
#endif
