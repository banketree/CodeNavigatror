...
int linphone_core_get_local_ip_for(int type, const char *dest, char *result){
	...
	LOG_RUNLOG_DEBUG("LP linphone_core_get_local_ip_for 1111111111111(%s)", result);
#ifdef HAVE_GETIFADDRS
	if (dest==NULL) {
	...
	LOG_RUNLOG_DEBUG("LP linphone_core_get_local_ip_for 22222222222");
#endif
	...
	LOG_RUNLOG_DEBUG("LP linphone_core_get_local_ip_for 3333333333");

	return get_local_ip_for_with_connect(type,dest,result);
}


static int get_local_ip_for_with_connect_bak(int type, const char *dest, char *result){
	int err,tmp;
	struct addrinfo hints;
	struct addrinfo *res=NULL;
	struct sockaddr_storage addr;
	...
	/*hints.ai_flags=AI_NUMERICHOST|AI_CANONNAME;*/
	err=getaddrinfo(dest,"8000",&hints,&res);
	if (err!=0){
		ms_error("getaddrinfo() error: %s",gai_strerror(err));
		return -1;
	}
...
//	sock=socket(res->ai_family,SOCK_DGRAM,0);
	...
	tmp=1;
//	bool_t bDontLinger = FALSE;
	err=setsockopt(sock,SOL_SOCKET,SO_REUSEADDR,(SOCKET_OPTION_VALUE)&tmp,sizeof(int));
//	err=setsockopt(sock,SOL_SOCKET,SO_KEEPALIVE,(SOCKET_OPTION_VALUE)&tmp,sizeof(int));
	if (err<0){
		ms_warning("Error in setsockopt: %s",strerror(errno));
	}
	err=connect(sock,res->ai_addr,res->ai_addrlen);
...
int linphone_core_get_local_ip_for_bak(int type, const char *dest, char *result){
	strcpy(result,type==AF_INET ? "127.0.0.1" : "::1");
#ifdef HAVE_GETIFADDRS
	if (dest==NULL) {
		/*we use getifaddrs for lookup of default interface */
