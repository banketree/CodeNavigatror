...
static MSList *match_payloads(const MSList *local, const MSList *remote, bool_t reading_response, bool_t one_matching_codec){
	...
	LOG_RUNLOG_DEBUG("LP CFIMC match_payloads begin");
	for(e2=remote;e2!=NULL;e2=e2->next){
		PayloadType *p2=(PayloadType*)e2->data;
		LOG_RUNLOG_DEBUG("LP CFIMC match_payloads Remote PayloadType %s", p2->mime_type);
	}

	for(e2=remote;e2!=NULL;e2=e2->next){
		LOG_RUNLOG_DEBUG("LP CFIMC match_payloads Match_payloads 1 ==> will match for "AUDIO_CODEC_FIRST);
		PayloadType *p2=(PayloadType*)e2->data;
		matched=find_payload_type_best_match(local,p2);
		if (matched &&  (strcasecmp(p2->mime_type, AUDIO_CODEC_FIRST) == 0) ){
			ms_message("Matched %s", p2->mime_type);
			LOG_RUNLOG_DEBUG("LP CFIMC match_payloads codec matched 1 %s", p2->mime_type);
			PayloadType *newp;
	...
	for(e2=remote;e2!=NULL;e2=e2->next){
		LOG_RUNLOG_DEBUG("LP CFIMC match_payloads Match_payloads 2 ==> will not match for "AUDIO_CODEC_FIRST);
		PayloadType *p2=(PayloadType*)e2->data;
		matched=find_payload_type_best_match(local,p2);
		if (matched &&  (strcasecmp(p2->mime_type, AUDIO_CODEC_FIRST) != 0) ){
			ms_message("Matched %s", p2->mime_type);
			LOG_RUNLOG_DEBUG("LP CFIMC match_payloads codec matched 2 %s(mode:%d/width:%d)", p2->mime_type, g_stIdbtCfg.iNegotiateMode, getCFimcAutoModeWidth());
//×¢ÊÍµôË½ÓÐÐ­Òé
//			if( strcasecmp(p2->mime_type, "H264") == 0 )
//			{
//				LOG_RUNLOG_DEBUG("LP CFIMC match_payloads H264 info recv:%s send:%s", p2->recv_fmtp, p2->send_fmtp);
//				char *tmp = strstr(p2->send_fmtp, "max-br=");
//				if(tmp != NULL)
//				{
//					LOG_RUNLOG_DEBUG("LP CFIMC match_payloads H264 info sendfmtp has video mode info %s", tmp+=strlen("max-br="));
//					if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() != MS_VIDEO_SIZE_CIF_W && strcmp(tmp, "192") == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 1  audomode is not cif will change to CIF\n");
//						CFimcReInitVideo(MS_VIDEO_SIZE_CIF_W, MS_VIDEO_SIZE_CIF_H);
//					}
//					else if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() == MS_VIDEO_SIZE_4CIF_W && strcmp(tmp, "512") == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 1  audomode is d1 no need to change\n");
//					}
//					else if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() != MS_VIDEO_SIZE_4CIF_W && strcmp(tmp, "512") == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 1  audomode is not d1 will change to D1\n");
//						CFimcReInitVideo(MS_VIDEO_SIZE_4CIF_W, MS_VIDEO_SIZE_4CIF_H);
//					}
//					else if(g_stIdbtCfg.iNegotiateMode == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 0  will use manual mode\n");
//					}
//				}
//				else
//				{
//					LOG_RUNLOG_DEBUG("LP CFIMC match_payloads did't find max-br=");
//				}

//				tmp = strstr(p2->send_fmtp, "video-mode=");
//				if(tmp != NULL)
//				{
//					LOG_RUNLOG_DEBUG("LP CFIMC match_payloads H264 info sendfmtp has video mode info %s", tmp+=strlen("video-mode="));
//					if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() != MS_VIDEO_SIZE_CIF_W && strcmp(tmp, "0") == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 1  audomode is not cif will change to CIF\n");
//						CFimcReInitVideo(MS_VIDEO_SIZE_CIF_W, MS_VIDEO_SIZE_CIF_H);
//					}
//					else if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() == MS_VIDEO_SIZE_4CIF_W && strcmp(tmp, "1") == 0)
//					{
//						LOGE("LP CFIMC iNegotiateMode == 1  audomode is d1 no need to change\n");
//					}
//					else if(g_stIdbtCfg.iNegotiateMode == 1 && getCFimcAutoModeWidth() != MS_VIDEO_SIZE_4CIF_W && strcmp(tmp, "1") == 0)
...
//				else
//				{
//					LOG_RUNLOG_DEBUG("LP CFIMC match_payloads did't find video-mode=");
//				}
//			}
	...
	LOG_RUNLOG_DEBUG("LP CFIMC match_payloads end");
	return res;
}
...
int offer_answer_initiate_outgoing(const SalMediaDescription *local_offer,
    ...
    for(i=0,j=0;i<local_offer->nstreams;++i){
		ms_message("offer_answer_initiate_outgoing Processing for stream %i",i);
...
int offer_answer_initiate_incoming(const SalMediaDescription *local_capabilities,
    ...
    for(i=0;i<remote_offer->nstreams;++i){
		...
		ms_message("offer_answer_initiate_incoming Processing for stream %i",i);
