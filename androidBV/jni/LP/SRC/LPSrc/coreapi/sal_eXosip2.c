...
//	eXosip_set_option(EXOSIP_OPT_USE_RPORT,&use_rports);
	//int dont_use_101 = 0; // Copy char to int to avoid bad alignment
	...
	//add by chensq 20121113
//	int dont_use_101 = 1; // Copy char to int to avoid bad alignment
//	eXosip_set_option(EXOSIP_OPT_DONT_SEND_101,&dont_use_101);
...
int sal_call(SalOp *h, const char *from, const char *to){
	...
	LOG_RUNLOG_INFO("LP SIP send method    ==> %s", invite->sip_method);
...
int sal_call_notify_ringing(SalOp *h, bool_t early_media){
	...
	if (early_media && h->sdp_answer){
		...
		if (msg){
			...
			LOG_RUNLOG_INFO("LP SIP send method    ==> %d", 180);
		}
		msg=NULL;
		...
		if (msg){
			...
			LOG_RUNLOG_INFO("LP SIP send method    ==> %d", 183);
		}
		eXosip_unlock();
	}else{
		eXosip_lock();
...
int sal_call_accept(SalOp * h){
	...
	LOG_RUNLOG_INFO("LP SIP send method    ==> %d", 200);
...
int sal_call_decline(SalOp *h, SalReason reason, const char *redirect){
	if (reason==SalReasonBusy){
		...
		LOG_RUNLOG_INFO("LP SIP send method    ==> %d", code);
		eXosip_unlock();
...
int sal_call_accept_refer(SalOp *op){
	...
	if(msg != NULL)
	{
		...
		LOG_RUNLOG_INFO("LP SIP send method    ==> %s", "NOTIFY");
...
int sal_call_refer(SalOp *h, const char *refer_to){
	...
	if (msg)
	{
		...
		LOG_RUNLOG_INFO("LP SIP send method    ==> %s", "REFER");
	}
	else err=-1;
	eXosip_unlock();
	return err;
...
int sal_call_send_dtmf(SalOp *h, char dtmf){
	...
	if (msg){
		...
		LOG_RUNLOG_INFO("LP SIP send method    ==> %s (dtmf:%c)", "INFO", dtmf);
	}
	eXosip_unlock();
	return 0;
}
...
int sal_call_terminate(SalOp *h){
	...
	LOG_RUNLOG_INFO("LP SIP send method    ==> %s", "TERMINATE(CANCEL, BYE or 603 Decline)");
	eXosip_unlock();
...
SalOp *find_op(Sal *sal, eXosip_event_t *ev){
	if (ev->cid>0){
...
static void call_accepted(Sal *sal, eXosip_event_t *ev){
	...
	LOG_RUNLOG_INFO("LP SIP send method    ==> %s", msg->sip_method);
...
static bool_t process_authentication(Sal *sal, eXosip_event_t *ev){
	...
	if (get_auth_data(ev,&realm,&username)==0){
		...
		//LOG_RUNLOG_DEBUG("LP process_authentication 1 %s %s", username, realm);
		return FALSE;
	}
	//LOG_RUNLOG_DEBUG("LP process_authentication 2 %s %s", username, realm);
	return TRUE;
}
...
static bool_t call_failure(Sal *sal, eXosip_event_t *ev){
	...
	switch(code)
	{
		case 401:
		case 407:
			//LOG_RUNLOG_INFO("LP call_failure process_authentication");
...
void sal_call_send_vfu_request(SalOp *h){
	...
	if (msg){
		...
		osip_message_set_content_length(msg,clen);
		eXosip_call_send_request(h->did,msg);
		LOG_RUNLOG_INFO("LP SIP send method    ==> %s (vfu request)", "INFO");
...
static void process_media_control_xml(Sal *sal, eXosip_event_t *ev){
	...
	/*in all other cases we must say it is not implemented.*/
	{
		...
		if (ans)
		{
			...
			LOG_RUNLOG_INFO("LP SIP send method    ==> %d", 501);
		}
...
static void process_refer(Sal *sal, SalOp *op, eXosip_event_t *ev){
	...
	if (h){
		...
		if (ans)
		{
			...
			LOG_RUNLOG_INFO("LP SIP send method    ==> %d", 202);
		}
...
static bool_t registration_failure(Sal *sal, eXosip_event_t *ev){
	...
	switch(status_code){
		case 401:
		case 407:
			//LOG_RUNLOG_INFO("LP registration_failure process_authentication");
			return process_authentication(sal,ev);
			break;
...
static bool_t process_event(Sal *sal, eXosip_event_t *ev){
	...
	if (pfEventProcess)
	{
		//LOG_RUNLOG_DEBUG("LP process_event:%d", ev->type);
		pfEventProcess(sal, ev);
	}
	...
	switch(ev->type){
		...
		case EXOSIP_CALL_MESSAGE_REQUESTFAILURE:
			if (ev->response &&
				...
				//LOG_RUNLOG_INFO("LP process_event process_authentication 3");
				 return process_authentication(sal,ev);
			}
			break;
		...
		case EXOSIP_SUBSCRIPTION_REQUESTFAILURE:   /**< announce a request failure      */
			if (ev->response && (ev->response->status_code == 407 || ev->response->status_code == 401)){
				//LOG_RUNLOG_INFO("LP process_event process_authentication 2");
				return process_authentication(sal,ev);
			}
...
#define MAX_CHANGE_VALUE (6000)
#define INIT_CHANGE_VALUE (5061)
void sal_eXosip2_ports_change()
{
	//add by chensq
	static int iChangeTimes = INIT_CHANGE_VALUE;
	static char acCmdBuff[32] = {0x00};

	if(iChangeTimes > MAX_CHANGE_VALUE)
	{
		iChangeTimes = INIT_CHANGE_VALUE;
		LOG_RUNLOG_ERROR("LP Could not build initial register too many times");
		return;
	}
	LOG_RUNLOG_WARN("LP Could not build initial register, will change port to %d", iChangeTimes);
	sprintf(acCmdBuff, "ports sip %d", iChangeTimes++);
	LPProcessCommand(acCmdBuff);
	LPProcessCommand("unregister");
	LPProcessCommand("register");
}

//modify by chensq 20120520 添加函数参数route以及支持fjims的代码
int sal_register(SalOp *h, const char *proxy, const char* route, const char *from, int expires){
	...
	if (h->rid==-1){
		...
		//"sip:+8659188244165@fj.ctcims.cn"
		//"sip:fj.ctcims.cn"
		//modify by chensq 20121114
		...
		if (msg){
			...
			sal_register_add_route(msg,proxy); //add by chensq 20121114
			...
			eXosip_unlock();
// || defined(_YK_PC_)
//#if defined(_YK_IMX27_AV_) || defined(_YK_PC_)
			//1.修复BV,IMX27在DHCP后SIP再也注册不上现象
			//2.修复IMX27在网线拔掉后SIP再也注册不上现象
			//add by chensq
...
////#if defined(REG_FJ_CTCIMS_CN)
//	if( strncmp(proxy+4, LP_IMS_SERVER_REALM, strlen(LP_IMS_SERVER_REALM)) == 0 )
//	{
//        if (route)
//        {
//            char buf[256]={0x0};
//            sprintf(buf, "<%s;lr>", route);
//            osip_message_set_route(msg, buf);
//        }
//        printf("debug:fj.ctcims.cn sal_register\n");
//	}
//	else
//	{
//	    printf("debug:normal sal_register\n");
//	}
//
////#endif
        //--------------------------------------------------------------------------------
...
int sal_call_update(SalOp *h, const char *subject){
	...
	LOG_RUNLOG_INFO("LP SIP send method    ==> %s (%s)", "INVITE", "reinvite");
	eXosip_unlock();
	return err;
}
