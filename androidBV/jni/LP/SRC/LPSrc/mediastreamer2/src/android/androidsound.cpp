...
static volatile int g_firstReadFlg = 0;
...
static AndroidSndWriteData *g_ad = NULL;
static volatile int g_doflg = 1;


extern "C" void android_snd_card_set_playflg(int doflg)
{
	g_doflg = doflg;
...
static void android_snd_card_detect(MSSndCardManager *m){
	LOGD("AndroidAudio android_snd_card_detect for LP");
	...
	if (libmedia && libutils){
		...
		if (audio_record_loaded && audio_track_loaded && audio_system_loaded && string8_loaded){
			LOGD("Native android sound support available.");
			MSSndCard *card=android_snd_card_new();
			ms_snd_card_manager_add_card(m,card);
			return;
		}
	}
	LOGE("Native android sound support is NOT available.");
...
static void android_snd_read_postprocess(MSFilter *obj){
	AndroidSndReadData *ad=(AndroidSndReadData*)obj->data;
	LOGV("LP ansound function=%s",__FUNCTION__);
	//g_doflg = 0;
	LOGV("LP ansound Stopping sound capture");
	if (ad->rec!=0) {
		ad->started=false;
		ad->rec->stop();
		delete ad->rec;
...
static void android_snd_write_process(MSFilter *obj){
	AndroidSndWriteData *ad=(AndroidSndWriteData*)obj->data;
//	LOGV("LP ansound android_snd_write_process in\n");
//		LOGV("LP ansound function=%s",__FUNCTION__);
...
#else
	ms_bufferizer_put_from_queue2(&ad->bf,obj->inputs[0], g_doflg);
	//ms_bufferizer_put_from_queue(&ad->bf,obj->inputs[0]);
	//LOGD("LP ansound ms_bufferizer_put_from_queue[%d]", g_doflg);
#endif
	...
	ms_warning("android_snd_write_process out\n");
...
static void android_snd_write_postprocess(MSFilter *obj){
	...
	if (!ad->tr) return;
		LOGV("LP ansound Stopping sound playback");
