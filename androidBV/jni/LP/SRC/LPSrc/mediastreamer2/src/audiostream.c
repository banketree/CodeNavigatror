...
void audio_stream_free(AudioStream *stream)
{
	...
	//20120525 dtmf inband 
    if (stream->dtmfinband) ms_filter_destroy(stream->dtmfinband);
	...
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
	...
	if (rtp_profile_get_payload_from_mime (profile,"telephone-event")==NULL
		...
		//20120525 dtmf inband
//		LOG_RUNLOG_DEBUG("LP g711 check dtmfinband(%s)", pt->mime_type);
		stream->dtmfinband = ms_filter_new(MS_DTMF_INBAND_ID);
	}
//	//add by chensq for g729 20120913
//	else if(strcasecmp(pt->mime_type,"g729")==0)
//	{
//		printf("###########################debug:g729 check dtmfinband(%s), %s, %d\n", pt->mime_type, __func__, __LINE__);
//		stream->dtmfinband = ms_filter_new(MS_DTMF_INBAND_ID);
//	}
//	//add by chensq for g729 20120913
//	else if(strcasecmp(pt->mime_type,"gsm")==0)
//	{
//		printf("###########################debug:gsm check dtmfinband(%s), %s, %d\n", pt->mime_type, __func__, __LINE__);
//		stream->dtmfinband = ms_filter_new(MS_DTMF_INBAND_ID);
//	}
	...
	//20120525 dtmf inband 
	if (stream->dtmfinband){
	    ms_connection_helper_link(&h,stream->dtmfinband,0,0);
	}
...
void audio_stream_stop(AudioStream * stream)
{
	if (stream->ticker){
		...
		//20120525 dtmf inband 
		if(stream->dtmfinband){
			ms_connection_helper_unlink(&h,stream->dtmfinband,0,0);
		}
