...
static void video_capture_detect(MSWebCamManager *obj){
	LOGD("Detecting Android VIDEO cards");
	//JNIEnv *env = ms_get_jni_env();
	JNIEnv *env = g_env;
	...
	for(int i=0; i<count; i++) {
		...
		LOGD("camera created: id=%d frontFacing=%d orientation=%d [msid:%s]\n", c->id, c->frontFacing, c->orientation, idstring);
	}

	env->DeleteGlobalRef(helperClass);
	LOGD("Detection of Android VIDEO cards done");
}
