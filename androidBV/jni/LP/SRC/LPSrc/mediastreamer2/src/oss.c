...
static void oss_open(OssData* d, int *minsz){
	...
	if (d->pcmfd_read > 0) {
		...
		LOG_RUNLOG_ERROR("LP IMX27 Cannot open fd in ro mode for (%s)",d->pcmdev);
		LOG_EVENTLOG_INFO(LOG_EVENTLOG_FORMAT, LOG_EVENTLOG_NO_ROOM, LOG_EVT_SYSTEM_ERROR, 0, "105");
	...
	if (d->pcmfd_write > 0) {
		...
		LOG_RUNLOG_ERROR("LP IMX27 Cannot open wr in ro mode for (%s)",d->pcmdev);
		LOG_EVENTLOG_INFO(LOG_EVENTLOG_FORMAT, LOG_EVENTLOG_NO_ROOM, LOG_EVT_SYSTEM_ERROR, 0, "105");
		LOG_EVENTLOG_INFO(LOG_EVENTLOG_FORMAT, LOG_EVENTLOG_NO_ROOM, LOG_EVT_APP_REBOOT, 0, "");
		LOGUploadResPrepross();
		LOGUploadExec(FP_TRANSFER_NOWAIT);
...
//#define OSS_PLAY_DEBUG
//#define OSS_REC_DEBUG
static void * oss_thread(void *p){
	printf("debug:oss thread begin\n");

#ifdef	_YK_IMX27_AV_
	unsigned char *pcPcmBuff = NULL;
	int iCount = 0;
#endif

#ifdef OSS_PLAY_DEBUG
	FILE *pstPlay;
	if ((pstPlay = fopen("/oss_play", "wb")) == NULL)
	{
		LOG_RUNLOG_ERROR("LP IMX27 create play file error");
	}
#endif

#ifdef OSS_REC_DEBUG
	FILE *pstRec;
	if ((pstRec = fopen("/oss_rec", "wb")) == NULL)
	{
		LOG_RUNLOG_ERROR("LP IMX27 create rec file error");
	}
#endif
...
#ifdef	_YK_IMX27_AV_
	pcPcmBuff = (unsigned char *)malloc(bsize+10);
	//printf("====================malloc %d\n", bsize+10);
	memset(pcPcmBuff, 0x00, bsize+10);
	if(pcPcmBuff == NULL)
	{
		LOG_RUNLOG_ERROR("LP IMX27 oss malloc error");
		return NULL;
	}
#endif
		...
		if (d->pcmfd_read>=0){
			if (d->read_started){
...
#ifdef	_YK_IMX27_AV_
				memset(pcPcmBuff, 0x00, bsize+10);
				err=read(d->pcmfd_read,pcPcmBuff,bsize);
#else
				...
				if (err<0){
					...
					while( iCount < err )
					{
						if( *(pcPcmBuff + iCount)  == 0x80 )
						{
							*(pcPcmBuff + iCount) = 0x00;
						}
						iCount++;
					}
					memcpy(rm->b_wptr, pcPcmBuff, err);
#endif
#ifdef OSS_REC_DEBUG
					fwrite(rm->b_wptr, bsize, 1, pstRec);
#endif
					did_read=TRUE;
					rm->b_wptr+=err;
...
#ifdef	_YK_IMX27_AV_
				memset(pcPcmBuff, 0x00, bsize+10);
				int sz=read(d->pcmfd_read,pcPcmBuff,bsize);
#else
				int sz=read(d->pcmfd_read,rtmpbuff,bsize);
#endif
...
#ifdef OSS_PLAY_DEBUG
					fwrite(wtmpbuff, bsize, 1, pstPlay);
#endif
					if (err<0){
