...
unsigned int g_abrc_level_index = ABRC_LEVEL - 1;
...
abrc_para g_abrc_level[ABRC_LEVEL] =
{
		{2 ,32 ,0},
		{5 ,64 ,0},
		{7 ,96 ,0},
...
{
	static float 		total_lost 		= 0;
	static float        average_lost    = 0;
	static unsigned int total_jitter 	= 0;
	static unsigned int average_jitter  = 0;
    ...
    average_lost   = total_lost   / AVERAGE_COUNT;
    average_jitter = total_jitter / AVERAGE_COUNT;

    LOG_RUNLOG_DEBUG("LP recv video stream  RTCP  interarrival jitter=%u , lost packets percentage since last report=%f ",average_jitter,average_lost);

	if( average_lost > LOST_MAX )
	{
		if(g_abrc_level_index > 0)
		{
			g_abrc_level[g_abrc_level_index].last_jitter = average_jitter;
			g_abrc_level_index--;
			ms_filter_call_method(stream->source,MS_FILTER_SET_BITRATE,&g_abrc_level[g_abrc_level_index]);
		}
	}
	else if ( average_lost < LOST_MIN)
	{
		if( g_abrc_level_index < (ABRC_LEVEL - 1) )
		{
			if( average_jitter > g_abrc_level[g_abrc_level_index+1].last_jitter)
			{
				return;
			}
...
static void video_steam_process_rtcp(VideoStream *stream, mblk_t *m)
{
	...
	do{
		if (rtcp_is_RR(m))
		{
			ms_message("video_steam_process_rtcp: receiving RTCP RR");
			...
			if (rb)
			{
				...
				//if (stream->adapt_bitrate)
					video_stream_adapt_bitrate(stream,ij,flost);
			}
		}
...
static void configure_video_source(VideoStream *stream)
{
	...
	float 			fps=15;
	MSPixFmt 		format;
	//对Encoder配置，从编码器中读vsize,与stream中的vsize比较选择合适的尺寸，而后读取fps,屏蔽
	/*
	...
	int rem_rtcp_port, int payload, int jitt_comp, MSWebCam *cam)
{
	...
	if (cam==NULL)//获取默认的视频采集设备
	{
		cam=ms_web_cam_manager_get_default_cam( ms_web_cam_manager_get());
	}

#ifdef _YK_MEDIA_ABRC_
	g_abrc_level_index = ABRC_LEVEL - 1;
	count              = 1;
#endif
	pt=rtp_profile_get_payload(profile,payload); //读取相应的payloadtype
	if (pt==NULL)
	{
	...
	//设置RTP会话参数信息
	rtp_session_set_profile(rtps,profile);		//设置RTP 会话参数
	if (remport>0) 
		rtp_session_set_remote_addr_full(rtps,remip,remport,rem_rtcp_port);
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamSendOnly)
	{
		...
		//根据payloadtype的参数值 调用Methord设置编码器，屏蔽
		/*
		...
		*/
		//调用Methord，对source,pixconv,sizeconv,encoder配置
