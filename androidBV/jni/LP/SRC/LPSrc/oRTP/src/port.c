...
#define TEMP_PATH				"/data/data/com.fsti.ladder"

static char *make_pipe_name(const char *name){
	return ortp_strdup_printf("/data/data/com.fsti.ladder/%s",name);
}
...
ortp_socket_t ortp_server_pipe_create(const char *name){
	...
	ortp_message("ortp_server_pipe_create, pipename:%s\n", pipename);

	sock=socket(AF_UNIX,SOCK_STREAM,0);
	if(sock == -1)
	{
		ortp_message("Failed to create server unix socket: %s",strerror(errno));
	}
	else
	{
		ortp_message("server sock is:%d",sock);
	}
	...
	if(fchmod(sock,S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0)
	{
		ortp_message("fchmod failed\n");
	}

	ortp_message("begin to bind\n");

	if (bind(sock,(struct sockaddr*)&sa,sizeof(sa))!=0){
		ortp_message("Failed to bind command unix socket: %s",strerror(errno));
		return -1;
	}
	ortp_message("before sock handle:%d \n", sock);
	listen(sock,1);
	ortp_message("after sock handle:%d \n", sock);
	return sock;
}
...
ortp_socket_t ortp_client_pipe_connect(const char *name){
	...
	ortp_message("ortp_client_pipe_create, pipename:%s\n", pipename);

	ortp_socket_t sock=socket(AF_UNIX,SOCK_STREAM,0);
	if(sock == -1)
	{
		ortp_message("Failed to create client unix socket: %s",strerror(errno));
	}
	else
	{
		ortp_message("client sock is:%d",sock);
	}
	sa.sun_family=AF_UNIX;
	strncpy(sa.sun_path,pipename,sizeof(sa.sun_path)-1);
	ortp_free(pipename);
	if (connect(sock,(struct sockaddr*)&sa,sizeof(sa))!=0){
		ortp_message("Failed to connect unix socket: %s",strerror(errno));
		close(sock);
		return -1;
	}

	ortp_message("ortp_client_pipe_connect finish");
	return sock;
}
