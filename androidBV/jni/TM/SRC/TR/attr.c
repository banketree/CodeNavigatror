// $Id: attr.c 12 2011-02-18 04:05:43Z cedric.shih@gmail.com $
/*
...
#define EVCPE_PHONECOUNT_STR   "PhoneCount"
#define EVCPE_CARDINFO_STR   "CardInfo"
#define EVCPE_CARDCOUNT_STR   "CardCount"
...
int evcpe_attr_add_obj2(struct evcpe_attr *attr,
		struct evcpe_obj **child, unsigned int *index, unsigned int *num)
{
	struct evcpe_obj_item *iter, *item;
	struct evcpe_attr *attrtmp;
	struct evcpe_obj *obj;
	struct evcpe_obj_item *iten;
	const char *value;
	long lvalue;
	unsigned int len;
	int rc;
	...
	*num = 0;
	...
	if (iten) {
		obj = iten->obj;
		if ((rc = evcpe_obj_get(iten->obj, EVCPE_NUM_STR, strlen(EVCPE_NUM_STR), &attrtmp))) {
			evcpe_error(__func__, "failed to get attribute: %.*s",
				strlen(EVCPE_NUM_STR), EVCPE_NUM_STR);
			rc = ENOMEM;
			goto finally;
		}
		if ((rc = evcpe_attr_get(attrtmp, &value, &len))) {
	...
	*child = item->obj;
	...
	/*if ((rc = evcpe_obj_get(item->obj, EVCPE_NUM_STR, strlen(EVCPE_NUM_STR), &attrtmp))) {
		evcpe_error(__func__, "failed to get attribute: %.*s",
			strlen(EVCPE_NUM_STR), EVCPE_NUM_STR);
		rc = ENOMEM;
		goto finally;
	...
	if ((rc = evcpe_attr_set(attrtmp,  &value, strlen(value)))) {
		evcpe_error(__func__, "failed to set value: %s", value);
		rc = EINVAL;
	}
	*index = num;*/
	rc = 0;
...
//----------------add by zlin
int evcpe_attr_del_obj_info(struct evcpe_attr *attr, unsigned int index)
{
	struct evcpe_obj *obj;
	struct evcpe_obj_item *item;
	int rc;
	...
	TAILQ_REMOVE(&attr->value.multiple.list, item, entry);//add by zlin
...
int evcpe_attr_del_obj5(struct evcpe_attr *attr, unsigned int index)
{
	//struct evcpe_obj *obj;
	struct evcpe_obj_item *item;
	struct evcpe_attr *attrtmp;
	...
	if ((rc = evcpe_obj_get(item->obj, EVCPE_PHONEINFO_STR, strlen(EVCPE_PHONEINFO_STR), &attrtmp))) {
		evcpe_error(__func__, "failed to get attribute: %.*s",
				strlen(EVCPE_PHONEINFO_STR), EVCPE_PHONEINFO_STR);
		goto cardinfo;
	}
	num = attrtmp->value.multiple.size;
	//num = evcpe_obj_list_get_num(&attrtmp->value.multiple.list);
	for (i = num; i>0 ; i--) {
		if ((rc = evcpe_attr_del_obj_info(attrtmp, i-1))) {
			evcpe_error(__func__, "failed to delete object: %s.%d", EVCPE_PHONEINFO_STR, i-1);
			rc = EINVAL;
			goto finally;
		}
	}
	...
	/*if ((rc = evcpe_obj_get(item->obj, EVCPE_PHONECOUNT_STR, strlen(EVCPE_PHONECOUNT_STR), &attrtmp))) {
		evcpe_error(__func__, "failed to get attribute: %.*s",
				strlen(EVCPE_PHONECOUNT_STR), EVCPE_PHONECOUNT_STR);
		rc = EINVAL;
		goto finally;
	}
	value = evcpe_ltoa(0);
	if ((rc = evcpe_attr_set(attrtmp, value, strlen(value)))) {
		evcpe_error(__func__, "failed to get value: %s", EVCPE_PHONECOUNT_STR);
		rc = EINVAL;
		goto finally;
	}*/

cardinfo:
	if ((rc = evcpe_obj_get(item->obj, EVCPE_CARDINFO_STR, strlen(EVCPE_CARDINFO_STR), &attrtmp))) {
		evcpe_error(__func__, "failed to get attribute: %.*s",
				strlen(EVCPE_CARDINFO_STR), EVCPE_CARDINFO_STR);
		rc = 0;
		goto finally;
	}
	num = attrtmp->value.multiple.size;
	...
	for (i = num;  i>0; i--) {
		if ((rc = evcpe_attr_del_obj_info(attrtmp, i-1))) {
			evcpe_error(__func__, "failed to delete object: %s.%d", EVCPE_CARDINFO_STR, i-1);
			rc = EINVAL;
			goto finally;
		}
       }
	...
	/*if ((rc = evcpe_obj_get(item->obj, EVCPE_CARDCOUNT_STR, strlen(EVCPE_CARDCOUNT_STR), &attrtmp))) {
		evcpe_error(__func__, "failed to get attribute: %.*s",
				strlen(EVCPE_CARDCOUNT_STR), EVCPE_CARDCOUNT_STR);
		rc = EINVAL;
		goto finally;
	}
	value = evcpe_ltoa(0);
	if ((rc = evcpe_attr_set(attrtmp, value, strlen(value)))) {
		evcpe_error(__func__, "failed to get value: %s", EVCPE_CARDCOUNT_STR);
		rc = EINVAL;
		goto finally;
	}*/
	rc = 0;
...
//----------------------add by zlin
int evcpe_value_to_param_value_list(struct evcpe_param_value_list *list, const char *path, unsigned int pathlen, const char *value, unsigned int len, enum evcpe_type type)
{
	int rc;
	struct evcpe_obj_item *item;
	struct evcpe_param_value *param;
    
	evcpe_debug(__func__, "adding value to param value list: %s",
			path);
		...
		struct evcpe_param_value_list *list)
{
	...
	evcpe_debug(__func__, "adding attribute to param value list: %s",
       ...
       /*根据节点类型区分处理*/
	switch (attr->schema->type) {
	case EVCPE_TYPE_OBJECT:/*object类型*/
       /*把object的值放到列表中*/
		...
		break;
	case EVCPE_TYPE_MULTIPLE:/*multiple object类型*/
       /*遍历把多实例object的参数值放到列表中*/
