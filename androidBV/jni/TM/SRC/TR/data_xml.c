...
/*int evcpe_fault_to_xml(struct evcpe_fault *fault,
		...
		const char *node, struct evbuffer *buffer)
{
	...
	if ((rc = evcpe_xml_add_string(buffer, "SerialNumber", id->serial_number)))
		goto finally;
	if ((rc = evcpe_xml_add_string(buffer, "FirmwareVersion", id->firmware_version)))
		goto finally;
	if ((rc = evcpe_xml_add_string(buffer, "HardwareVersion", id->hardware_version)))
		goto finally;
	if ((rc = evcpe_xml_add_string(buffer, "KernelVersion", id->kernel_version)))
		goto finally;
	if ((rc = evcpe_xml_add_string(buffer, "DeviceNo", id->device_no)))
		goto finally;
	if ((rc = evcpe_add_buffer(buffer, "</%s>\n", node)))
		goto finally;
