// $Id: delete_object.c 12 2011-02-18 04:05:43Z cedric.shih@gmail.com $
/*
...
{
	struct evcpe_delete_object_response *method;

	evcpe_debug(__func__, "constructing evcpe_delete_object_response");

	if (!(method = calloc(1, sizeof(struct evcpe_delete_object_response)))) {
		evcpe_error(__func__, "failed to calloc evcpe_delete_obejct_response");
		return NULL;
	}
	return method;
}

void evcpe_delete_object_response_free(struct evcpe_delete_object_response *method)
{
	if (!method) return;
	evcpe_debug(__func__, "destructing evcpe_delete_object_response");
	free(method);
}
