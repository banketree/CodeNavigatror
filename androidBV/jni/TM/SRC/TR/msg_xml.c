...
int evcpe_msg_to_xml(struct evcpe_msg *msg, struct evbuffer *buffer)
{
	...
	case EVCPE_MSG_REQUEST:
		...
		case EVCPE_TRANSFER_COMPLETE:
			if ((rc = evcpe_transfercomplete_to_xml(msg->data, buffer))) {
				evcpe_error(__func__, "failed to marshal inform");
				goto finally;
			}
			break;
	...
	case EVCPE_MSG_RESPONSE:
		...
		case EVCPE_REBOOT:
			if ((rc = evcpe_reboot_response_to_xml(msg->data, buffer))) {
				evcpe_error(__func__, "failed to marshal reboot_response");
				goto finally;
			}
			break;
		case EVCPE_UPLOAD:
			if ((rc = evcpe_upload_response_to_xml(msg->data, buffer))) {
				evcpe_error(__func__, "failed to marshal upload_response");
				goto finally;
			}
			break;
		case EVCPE_DOWNLOAD:
			if ((rc = evcpe_download_response_to_xml(msg->data, buffer))) {
				evcpe_error(__func__, "failed to marshal download_response");
				goto finally;
			}
			break;
		default:
		...
		const char *ns, unsigned nslen, const char *name, unsigned len)
{
	...
	if (!evcpe_strncmp("Envelope", name, len)) {
		...
		if (!(parser->msg->data = evcpe_get_param_names_new()))
			return ENOMEM;
	} else if (!evcpe_strncmp("Download", name, len)) {
		if (evcpe_strncmp("Body", parent->name, parent->len)) {
			goto unexpected_parent;
		}
		parser->msg->type = EVCPE_MSG_REQUEST;
		...
		if (!(parser->msg->data = evcpe_upload_new()))
			return ENOMEM;
	} else if (!evcpe_strncmp("Reboot", name, len)) {
		if (evcpe_strncmp("Body", parent->name, parent->len)) {
			goto unexpected_parent;
		}
		parser->msg->type = EVCPE_MSG_REQUEST;
		...
		if (!(parser->msg->data = evcpe_transfercomplete_response_new()))
			return ENOMEM;
		parser->msg->type = EVCPE_MSG_RESPONSE;
		...
		if (evcpe_strncmp("AddObject", parent->name, parent->len) &&
			//-----------------------edit by zlin
				//evcpe_strncmp("DeleteObject", parent->name, parent->len)) {
				evcpe_strncmp("DeleteObject", parent->name, parent->len) &&
				evcpe_strncmp("SetParameterValues", parent->name, parent->len)){
			//------------------------
			goto unexpected_parent;
		}
	}
...
int evcpe_msg_xml_data_cb(void *data, const char *text, unsigned len)
{
	...
	//----------------------add by zlin
	} else if (!evcpe_strncmp("CommandKey", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_REBOOT:
			reboot = parser->list_item;
			break;
		case EVCPE_UPLOAD:
			upload = parser->msg->data;
			if (len >= sizeof(upload->commandkey)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->commandkey, text, len);
			upload->commandkey[len] = '\0';
			break;
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->commandkey)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->commandkey, text, len);
			download->commandkey[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("FileType", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_UPLOAD:
			upload = parser->msg->data;
			if (len >= sizeof(upload->file_type)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->file_type, text, len);
			upload->file_type[len] = '\0';
			break;
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->file_type)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->file_type, text, len);
			download->file_type[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("URL", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_UPLOAD:
			upload = parser->msg->data;
			if (len >= sizeof(upload->url)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->url, text, len);
			upload->url[len] = '\0';
			break;
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->url)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->url, text, len);
			download->url[len] = '\0';
	...
	} else if (!evcpe_strncmp("Username", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_UPLOAD:
			upload = parser->msg->data;
			if (len >= sizeof(upload->username)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->username, text, len);
			upload->username[len] = '\0';
			break;
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			...
			memcpy(download->username, text, len);
			download->username[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("Password", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_UPLOAD:
			upload = parser->msg->data;
			if (len >= sizeof(upload->password)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->password, text, len);
			upload->password[len] = '\0';
			break;
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->username)) {
	...
	} else if (!evcpe_strncmp("FileSize", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_DOWNLOAD:
			/*download = parser->msg->data;
			if (len >= sizeof(download->file_size)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->file_size, text, len);
			download->file_size[len] = '\0';*/
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("TargetFileName", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			...
			memcpy(download->target_filename, text, len);
			download->target_filename[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("DelaySeconds", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_UPLOAD:
			/*upload = parser->msg->data;
			if (len >= sizeof(upload->delay_seconds)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(upload->delay_seconds, text, len);
			upload->delay_seconds[len] = '\0';*/
			//upload = parser->list_item;
			break;
		case EVCPE_DOWNLOAD:
			download = parser->list_item;
			/*if ((rc = evcpe_set_param_value_set(download, text, len)))
				goto finally;*/
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
	...
	} else if (!evcpe_strncmp("SuccessURL", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->success_url)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->success_url, text, len);
			download->success_url[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	} else if (!evcpe_strncmp("FailureURL", elm->name, elm->len)) {
		switch (parser->msg->method_type) {
		case EVCPE_DOWNLOAD:
			download = parser->msg->data;
			if (len >= sizeof(download->failure_url)) {
				rc = EOVERFLOW;
				goto finally;
			}
			memcpy(download->failure_url, text, len);
			download->failure_url[len] = '\0';
			break;
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
	...
	//---------------------------
		...
		if (len != 1 || text[0] != '1') {
			err = -33;	//add by zlin
			evcpe_error(__func__, "MaxEnvelopes error: len=%d, text=%s",
					len, text);
			goto syntax_error;
		}
		...
		case EVCPE_DELETE_OBJECT:
			...
			break;
		case EVCPE_SET_PARAMETER_VALUES://add by zlin
			break;				//add by zlin
		default:
			evcpe_error(__func__, "unexpected evcpe_method_type: %d",
					parser->msg->method_type);
...
syntax_error:
	evcpe_error(__func__, "syntax error, err = %d", err);	//edit by zlin
	return EPROTO;
}
		...
		const char *value, unsigned value_len)
{
	...
	evcpe_debug(__func__, "attribute: %.*s => %.*s (namespace: %.*s)",
			name_len, name, value_len, value, nslen, ns);
	//------------------
