...
void evcpe_session_http_cb(struct evhttp_request *http_req, void *arg)
{
	...
	if(server_time)
	{
		//时间解析及设置 Tue, 26 Feb 2013 02:19:51 GMT
	...
	/*evcpe_session_handle_incoming处理报文后，获取报文发送消息队列的msg信息准备发送*/
...
int evcpe_session_start(struct evcpe_session *session)
{
	...
	/*调用evcpe_session_send_do 函数组装报文，准备发送inform请求*/
...
//#endif
	...
	/*创建一个新的http请求对象，并设置http回调处理函数evcpe_session_http_cb*/
	...
	//设置关闭回调函数，此时如果对端有响应，则转入evcpe_session_http_cb处理response*/
