...
#define cfg_name "/mnt/mtd/config/door26cfg"
...
struct Local1{
               ...
               int WatchDogClearFlag;   //WatchDog Clear 标志

               int CallKeyFlag;  //呼叫键标志     20110901  xu
               int CallKeyTime;  //呼叫键计数

               int TalkFinishFlag;  //通话结束标志     20101029  xu
               int TalkFinishTime;  //通话结束计数

               int CaptureVideoStartFlag;  //捕获视频开始标志     20101109  xu
               int CaptureVideoStartTime;  //捕获视频开始计数

               char cRegFlg;								//SIP呼叫状态 add by chensq 20120401
               char cPlayFlg;								//是否播放彩铃 add by chensq 20120401
               char cSipCallPassive;						// SIP主动发起呼叫
