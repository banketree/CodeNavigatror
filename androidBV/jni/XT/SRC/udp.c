...
//extern struct tempbuf1 tempbuf;     //视频临时环形缓冲   接收用

extern sem_t videorec2playsem;
extern struct tempvideobuf1 tempvideobuf[MP4VNUM]; //视频临时环形缓冲  接收用

//int temp_video_head;   //视频接收缓冲头部
//int temp_video_end;    //视频接收缓冲尾部
		...
		int length, int m_Socket)
{
	...
	if (strcmp(FlagText, FLAGTEXT) == 0)
	{
		...
		case 1: //开始
			...
			sprintf(Local.DebugInfo, "发送远程调试信息 开始, %s, Local.DebugIP = %s\n",
					...
					for (j = 0; j < Remote.DenNum; j++)
					{
						...
						for (i = 0; i < UDPSENDMAX; i++)
							if (Multi_Udp_Buff[i].isValid == 0)
							{
								...
								{
									sprintf(Local.DebugInfo,
											"解析地址成功,正在呼叫, Multi_Udp_Buff[i].RemoteHost is %s\n",
...
void RecvTalkLeaveGroup_Func(unsigned char *recv_buf, char *cFromIP)
{
	...
	if ((Local.Status == 2) && ((recv_buf[7] & 0x03) == ASK))
	{
		...
		sprintf(Local.DebugInfo, "对方要求离开组播组, %s\n", cFromIP);
		P_Debug(Local.DebugInfo);
	}
	else //本机为主叫方 发起
	if ((recv_buf[7] & 0x03) == REPLY)
								...
								if (Multi_Udp_Buff[i].buf[8] == CALL )
									if (strcmp(Multi_Udp_Buff[i].RemoteHost, cFromIP) == 0)
										if (strcmp(Multi_Udp_Buff[i].RemoteIP, RemoteIP) == 0)
										{
											...
											sprintf(Local.DebugInfo, "收到对讲呼叫应答, i = %d, %d.%d.%d.%d\n",
													i, recv_buf[53],
													recv_buf[54], recv_buf[55],
													recv_buf[56]);
											P_Debug(Local.DebugInfo);
			...
			|| (Local.Status == 9)) && ((recv_buf[7] & 0x03) == ASK))
	{
		...
		if (Local.Status == 5)
					...
					|| (Remote.IP[0][3] != recv_buf[56]))
			{
				sprintf(
						Local.DebugInfo,
						"单个被叫方退出组播组: Remote.IP = %d.%d.%d.%d, recv_buf = %d.%d.%d.%d, cFromIP = %s\n",
						Remote.IP[0][0], Remote.IP[0][1], Remote.IP[0][2],
...
void RecvTalkCallEnd_Func(unsigned char *recv_buf, char *cFromIP)
{
			...
			&& ((recv_buf[7] & 0x03) == ASK))
	{
		...
		//呼叫结束
#ifdef _DEBUG
		sprintf(Local.DebugInfo, "对方结束对讲, %s\n", cFromIP);
		P_Debug(Local.DebugInfo);
#endif
	}
	else //本机主动
...
//    ioctl(gpio_fd, IO_PUT, 1);
	...
	if (g_sipCallID == INVALID_SIP_CALL_ID)
	{
		...
		Local.TalkFinishFlag = 1; //通话结束标志     20101029  xu
		...
		for (j = 0; j < Remote.DenNum; j++)
		{
			...
			for (i = 0; i < UDPSENDMAX; i++)
				if (Multi_Udp_Buff[i].isValid == 1)
				{
					...
					{
						isIP = 1;
						sprintf(Local.DebugInfo, "退出组播组命令已在缓冲内, %s\n", tmp_ip);
						P_Debug(Local.DebugInfo);
						break;
					}
				}
				...
				for (i = 0; i < UDPSENDMAX; i++)
					if (Multi_Udp_Buff[i].isValid == 0)
					{
						...
						if (DebugMode == 1)
						{
							sprintf(
									Local.DebugInfo,
									"正在要求退出组播组, Multi_Udp_Buff[i].RemoteHost is %s\n",
									Multi_Udp_Buff[i].RemoteHost);
							P_Debug(Local.DebugInfo);
						}
						//头部
