...
int record_file(unsigned rate, unsigned channels, int fd, unsigned count)
{
    ...
    while (!pcm_read(pcm, data, bufsize) && (i < 100)) {
...
int rec_wav(const char *fn)
{
    ...
    hdr.byte_rate = hdr.sample_rate * hdr.num_channels * hdr.fmt_sz / 8;

    hdr.block_align = hdr.num_channels * hdr.fmt_sz / 8;
