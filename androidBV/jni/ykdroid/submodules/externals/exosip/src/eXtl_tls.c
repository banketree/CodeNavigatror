...
static int tls_tl_reset(void){
	int pos;
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tls_socket_tab[pos].socket!=0)
			tls_socket_tab[pos].invalid=1;
	}
	return 0;
}
...
int static print_ssl_error(int err)
{
	...
	case SSL_ERROR_SYSCALL:
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL, "SSL ERROR SYSCALL: %s\n",strerror(errno)));
		break;
	default:
		OSIP_TRACE(osip_trace
...
static int _tls_tl_is_connected_with_timeout(int sock, int millis)
{
	int res;
	struct timeval tv;
	fd_set wrset;
...
{
	return _tls_tl_is_connected_with_timeout(sock, SOCKET_TIMEOUT);
}
...
static int _tls_tl_check_connected()
{
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tls_socket_tab[pos].invalid){
			/*this socket was set to invalid, close it */
			OSIP_TRACE(osip_trace
							(__FILE__, __LINE__, OSIP_INFO2, NULL,
							"_tls_tl_check_connected: Socket node invalidated:%s:%i, socket %d [pos=%d], family:%d\n",
		...
		if (r <= 0) {
			...
			if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
				...
				OSIP_TRACE(osip_trace
							...
							NULL, "*** SSL_read returned %i, bufsize=%i\n",r,sockinfo->bufsize - sockinfo->buflen - rlen));
				OSIP_TRACE(osip_trace
							(__FILE__, __LINE__, OSIP_WARNING,
							NULL, "TLS closed\n"));
			...
			rlen += r;
			/*Break here. re-composition of message will occur later if it is fragmented. If not breaking here then the buffer might become full, causing SSL_read() to return 0 while the connection is still active.*/
                        break;
		}
	}
...
static int _tls_tl_connect_socket(char *host, int port)
{
    ...
    for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
        ...
        struct sockaddr *curaddr=(struct sockaddr *) curinfo->ai_addr;
        res = getnameinfo(curaddr, curinfo->ai_addrlen, src6host,
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		if (res < 0) {
			...
			if (ex_errno != EINPROGRESS) {
				...
				res = _tls_tl_is_connected_with_timeout(sock, SOCKET_PROGRESS_TIMEOUT);
				if (res > 0) {
					OSIP_TRACE(osip_trace
							   (__FILE__, __LINE__, OSIP_INFO2, NULL,
								"warning socket node:%s, socket %d [pos=%d], family:%d, still in progress: closing\n",
