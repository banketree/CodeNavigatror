...
void ms_filter_process(MSFilter *f){
	...
	long long iload;
	ms_warning("Executing process of filter %s:%p",f->desc->name,f);
	...
	if (f->stats){
		...
		f->stats->count++;
		iload = (stop.tv_sec-start.tv_sec)*1000000000LL + (stop.tv_nsec-start.tv_nsec);
		f->stats->elapsed+=(stop.tv_sec-start.tv_sec)*1000000000LL + (stop.tv_nsec-start.tv_nsec);
		ms_warning("Executing process of filter %s:load %l64d",f->desc->name,iload);
