...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private Timer flowTimer = new Timer();
...
//				LogUtils.printLog("HB timeSpan ：" + timeSpan + "now time :" + date);
				snapToScreen((mCurrentScreen + 1) % getChildCount());
			}
		};

		LogUtils.printLog("HB startAutoFlowTimer");
	    if (mTimerTask != null)
	    {
		...
		flowTimer.schedule(mTimerTask, timeSpan, timeSpan);
	}

	public void stopAutoFlowTimer() {
		if (mTimerTask != null)
	    {
			LogUtils.printLog("HB stopAutoFlowTimer");
	    	mTimerTask.cancel();  //将原任务从队列中移除
	    	mTimerTask = null;
        }
	}
	...
	private void resetFocus() {
		...
		for (int i = Math.max(0, mCurrentAdapterIndex - mSideBuffer); i < Math.min(mAdapter.getCount(), mCurrentAdapterIndex
				+ mSideBuffer + 1); i++) {
